...
class FramebufferDisplay:
    ...
    def render_status_screen(self, session_seconds, status, alerts, current_time):
        ...
        # Controls in footer
        self.draw_text_to_framebuffer(5, 227, "A:ACK  B:REFRESH", self.CYAN, 0x4208)
...
def run_framebuffer_monitor():
    ...
    while True:
        ...
        # Update display every second for smooth clock
        ...
        if button_a.value() == 0:
            if alerts_pending > 0:
                ...
                # Force immediate refresh to show change
                display.render_status_screen(session_seconds, status, alerts_pending, current_time)
                display.display_framebuffer()
